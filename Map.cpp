...
/*
 * Algorithm: Checks if the current location is an NPC location
 * get current row,col
 * loop i from 0 to npc_count_
 *      if (row,col) is a npc location
 *          return true
 ...
 * Return: boolean (bool)
 */
bool Map::currentIsNPC()
{
    int row = player_position_[0];
    int col = player_position_[1];
    for (int i = 0; i < npc_count_; i++)
    {
        if (row == npc_positions_[i][0] && col == npc_positions_[i][1])
        {
...
/*
 * Algorithm: Checks if the current location is an room location
 * get current row,col
 * loop i from 0 to room_count_
 *      if (row,col) is a room location
 *          return true
...
/*
 * Algorithm: Checks if the current location is an explored space
 * get current row,col
 * if map_data_[row][col] is ' ':
 *     return true
 * if (row, col) is npc location and has been found:
...
/*
 * Algorithm: checks if current location is dungeon_exit_
 * get current row,col
 * if (row,col) is dungeon exit:
 *      return true
 * return false
 */
bool Map::currentIsExit()
{
    int row = player_position_[0];
    int col = player_position_[1];
    if (row == dungeon_exit_[0] && col == dungeon_exit_[1])
...
/*
 * Algorithm: Checks if the current location on map is a free space
 * if row and column is a npc location
 *      return false
 * if row and column is a room location
 *      return false
